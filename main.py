...
from dataset import MyDataset, DownDataset, DownDatasetPre
...
def save_checkpoint(model, optimizer, args, epoch, down_model=False):
    ...
    if not down_model:
        ...
        }, os.path.join('checkpoints', 'epoch'+str(epoch)+'_pretrain12_20250424.pth'))
    else:
        torch.save({
            'model_state_dict': model_state_dict,
            'global_epoch': epoch,
        }, os.path.join('checkpoints', 'epoch' + str(epoch) + '_down12_20250424.pth'))
